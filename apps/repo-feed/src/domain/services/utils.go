...
func decodeZSetMember(member string) (uint, float64, time.Time, error) {
	...
	if len(parts) != 3 {
		utils.SugarLog.Errorf("invalid ZSET member format: expected 3 parts, got %d", len(parts))
	...
	if err != nil {
		utils.SugarLog.Errorf("failed to parse article ID '%s': %v", parts[0], err)
	...
	if err != nil {
		utils.SugarLog.Errorf("failed to parse original score '%s': %v", parts[1], err)
	...
	if err != nil {
		utils.SugarLog.Errorf("failed to parse publishedAt Unix time '%s': %v", parts[2], err)
