...
class SBERTArticleConsumerConfig(pydantic.BaseModel):
    article_vector_features: int
...
class SBERTArticleConsumer(InferenceConsumer):
    def __init__(
        ...
        config: SBERTArticleConsumerConfig,
    ):
        super().__init__(
            logger,
            consumer_config,
        ...
        self.qdrant = QdrantClient(
            url=config.qdrant_url, api_key=config.qdrant_api_key, prefer_grpc=False
        )
        self.config = config
        self._init_redis()
